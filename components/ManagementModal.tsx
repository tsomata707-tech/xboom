...
// Optimized with React.memo to prevent unnecessary re-renders and lag
const ManagementModal: React.FC<ManagementModalProps> = React.memo(({ 
    isOpen, onClose, onOpenAgentsModal, onOpenAvatarModal, onLogout, isAdmin, userProfile, onDisplayNameChange
}) => {
    ...
    // Prepare state only when modal opens to save resources when closed
    useEffect(() => {
        if (isOpen) {
